...
class TaxReceiptContainer(Protocol):
    """Cosmos container operations required by tax receipt storage."""

    create_item: Callable[..., dict[str, Any]]
    delete_item: Callable[..., None]
    read_item: Callable[..., dict[str, Any]]
    replace_item: Callable[..., dict[str, Any]]
    upsert_item: Callable[..., dict[str, Any]]
    query_items: Callable[..., Any]
...
def _is_cosmos_status_error(exc: Exception, status_code: int) -> bool:
    if not hasattr(exc, "status_code"):
        return False
    exc_status_code = getattr(exc, "status_code")
