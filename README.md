...
首頁、公開驗證頁與管理平台目前為 HTML 頁面；首頁活動清單、完訓證明查詢、志工服務證明申請、公開查詢限制與證明 QRCode 驗證已串接 Cosmos DB。
...
- 完訓證明查詢成功且 `certStatus` 為 `notIssued`、`changeRequested`、`issued` 或 `transferred` 時，首頁會顯示「選擇證明顯示方式」UI；若活動票種支援志工服務證明，會顯示「申請種類」並可切換為志工服務證明，切換後顯示服務開始日期、結束日期、服務時數與可填寫的單位名稱；`notIssued` 可進入「修改申請」並寫入 Cosmos DB，已轉移到尚未發行志工服務證明的資料也可提出修改申請；同一時間只有 pending 修改申請會阻擋再次提出，`approved` 或 `rejected` 完成審核後仍可再次送出；`changeRequested` 或 pending 申請會顯示處理中提示並隱藏「提出修改申請」；若已有已完成審核的修改申請，首頁會顯示通過或駁回結果，並在有審核備註時以第二行顯示 `審核備註`
...
| `GET` | `/verify/{certId}` | QRCode 入口的公開證明驗證頁面 | `text/html; charset=utf-8` |
...
- `BLOB_DOCUMENT_ASSETS_CONTAINER`、`BLOB_ISSUED_CERT_CONTAINER` 與 `BLOB_TAX_RECEIPTS_CONTAINER` 是 Blob Storage container 設定；目前 Azure 環境使用 `document-assets`、`issued-certs` 與 `tax-receipts`。完訓證明 PDF 底圖模板跟隨 git 版控，固定印章圖預設讀取 `document-assets/shared/organization-seal.png`，可用 `CERTIFICATE_ORGANIZATION_SEAL_BLOB_NAME` 覆寫；完訓證明樣式示意 PNG 從 `document-assets/completion-cert/previews/png/{locale}-{nameDisplay}-{org|no-org}.png` 讀取，志工服務證明樣式示意 PNG 從 `document-assets/volunteer-service-cert/previews/png/{locale}-{nameDisplay}-{org|no-org}.png` 讀取；樣式示意 PDF 備份分別存放於 `document-assets/completion-cert/previews/pdf/{locale}-{nameDisplay}-{org|no-org}.pdf` 與 `document-assets/volunteer-service-cert/previews/pdf/{locale}-{nameDisplay}-{org|no-org}.pdf` 並使用 Archive tier；產生後 PDF 會依文件類型寫入 `issued-certs/completionCert/{eventId}/{certId}.pdf` 或 `issued-certs/volunteerServiceCert/{eventId}/{certId}.pdf` 並指定為 Cool tier；營業稅繳稅證明原始檔會寫入 `tax-receipts`。若執行環境沒有 `AzureWebJobsStorage__accountName`，可用 `BLOB_STORAGE_ACCOUNT_NAME` 指定 Blob Storage account。
